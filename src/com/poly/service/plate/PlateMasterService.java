		...
		try {
			...
			for (String id : setin) {
				...
				String sql = "insert into tb_plate_master(`master_id`,`user_id`,`plate_id`,`add_time`,`is_delete`) "
												+ "values("+id+","+map.get("user_id")+","+map.get("plate_id")+",'"+ map.get("add_time") +"',"+map.get("is_delete")+")";
				jdbc.update(sql);
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (is_delete==1) {
						try {
							...
							jedis.zadd("tb_plate_master:list:plate_id:" + map2.get("plate_id"), timer, master_id + "");
							jedis.zadd("tb_plate_master:list:userid:plateid:" + map2.get("plate_id"), 0, map2.get("user_id"));
							jedis.zadd("tb_plate_master:list:user_id:" + map2.get("user_id"), timer, master_id + "");
							jedis.zadd("tb_plate_master:list:plateid:userid:" + map2.get("user_id"), timer, map2.get("plate_id"));
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
